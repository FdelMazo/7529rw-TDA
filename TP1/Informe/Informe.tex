...
\begin{itemize}
\item \texttt{Append} y \texttt{pop}: $\bigO(1)$
...
$\bigO(2 + 2 + 1 + 1 + (n-1) 3 + 1 + 1) = \bigO(3n + 5)) \in \bigO(n)$\\
...
La cantidad de llamadas recursivas (a) son 2. El tiempo de las llamadas no recursivas es $\bigO(1 + 3 + 1 + n (1 + 2) + 1 + n) = \bigO(4n + 6) \in \bigO(n)$, por lo que c=1.\\
...
Para poder terminar de ubicar en la escala a quicksort y mergesort, se pone el foco en la complejidad de su parte no recursiva: por parte de quicksort es $\bigO(4n + 6)$, y en el caso de mergesort es $\bigO(3n + 5)$. Es decir, la diferencia fundamental entre estos dos algoritmos es que en el peor caso quicksort será menos eficiente.
